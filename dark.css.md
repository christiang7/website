...
*make.sh*
```bash
noweb.py -Rdark.css dark.css.md > dark.css && echo 'dark.css' && notify-send -a "Compilation of dark.css" "" "$(date +"%Y-%m-%d") fertig" 
```


```bash
chmod u+x dark.css && ln -sf $(pwd)/dark.css ~/.local/bin/dark.css && echo 'fertig'
```
